...
if __name__ == "__main__":
    ...
    try:
        ...
        pass
    if len(sys.argv) < 4:
        print('Pass the server IP, port, image, output statistic filename as the sole command line argument.')
    else:
        ...
        for i in range(100):
            ...
            results = '{}\t\t{}\t\t{}\n'.format(sent_timestamp,
                                                RTT_time,
                                                abs(RTT_time - sent_timestamp))
            w.write(results)
            delay.append(abs(RTT_time - sent_timestamp))
